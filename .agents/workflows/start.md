---
description: Start all Neo-Lounge services (API on 3000, Kiosk on 3001, Tablet on 3002, PWA on 3003, Dashboard on 3004)
---
...
1. Kill any processes on ports 3000, 3001, 3002, 3003, 3004 using `fuser` (catches child processes reliably):
...
7. Start the Owner Dashboard (port 3004 — configured in package.json):

```bash
cd ~/devprojects/Neo-Lounge/apps/dashboard && npm run dev &
```

8. Wait for services to come up and verify all five ports are listening:
